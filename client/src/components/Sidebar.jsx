...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-40 flex flex-col bg-slate-900/95 lg:bg-slate-900 border-r border-slate-800 transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'
          } ${isOpenMobile
            ? 'translate-x-0'
      ...
      >
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-all duration-150 group ${isActive
                      ? 'bg-indigo-600/15 text-indigo-400 font-semibold border border-indigo-500/20 shadow-xs'
                      : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60'
                    }`}
                >
                  <Icon
                    className={`w-5 h-5 shrink-0 transition-colors ${isActive ? 'text-indigo-400' : 'text-slate-400 group-hover:text-slate-200'
                      }`}
                  />
            ...
            <div className="pt-2">
              ...
              <div className="space-y-1">
                  ...
                  projects.slice(0, 7).map((proj) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl text-xs font-medium transition-all ${isSelected
                ...
                {projects.length > 7 && (
                  ...
                  >
                    + {projects.length - 7} more projects...
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="p-3 border-t border-slate-800 bg-slate-900/60 shrink-0 space-y-2">
          ...
          {user && (
            <div
              className={`flex items-center gap-3 p-2 rounded-xl bg-slate-800/50 border border-slate-800 ${collapsed ? 'justify-center' : 'justify-between'
                }`}
            >
              <div
